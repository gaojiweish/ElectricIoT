...
namespace GK.TCPIPServer
{
    public class TSocketServerBase<TSession> : IDisposable, ISessionEvent
        where TSession : TSessionBase, new()
    {
        ...
        private int m_servertPort = 10001;
        ...
        {
            bool canCreateNew;
            m_ServerMutex = new Mutex(true, "GK_SERVER", out canCreateNew);
            if (!canCreateNew)
            {
        ...
        private bool CheckSocketIP(Socket clientSocket)
        {
            ...
            if (ip.Substring(0, 7) == "127.0.0.1")   // local machine
            {
                return true;
            }
                ...
                try
                {
                    if (m_serverSocket.Poll(m_acceptListenTimeInterval, SelectMode.SelectRead))
                    {
                        ...
                        if (clientSocket != null && clientSocket.Connected)
                        {
                            ...
                            if (m_sessionCount >= m_maxSessionCount)
                            {
